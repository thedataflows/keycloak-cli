...
`)
	...
	require.NoError(t, InstallDefaultFieldOverrides(filepath.Join(dir, "missing.spec.json")))
...
func TestInstallDefaultFieldOverridesWiresDefaultsWhenFileMissing(t *testing.T) {
	require.NoError(t, InstallDefaultFieldOverrides(filepath.Join(t.TempDir(), "missing.spec.json")))
